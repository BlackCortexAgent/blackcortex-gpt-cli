...
def test_command_update_with_pip(monkeypatch):
    ...
    with (
        ...
        patch("blackcortex_cli.commands.read_project_metadata", return_value={"name": "my-cli"}),
    ):
...
def test_command_update_failure():
    ...
    with (
        patch("blackcortex_cli.commands.read_project_metadata", return_value={"name": "cli"}),
...
def test_command_set_key_valid(monkeypatch):
    ...
    with (
        ...
        patch("blackcortex_cli.commands.ENV_PATH", os.path.join(tmpdir, ".env")),
        patch("blackcortex_cli.commands.console.print") as mock_print,
    ):
...
def test_command_ping_success():
    ...
    mock_client.models.list.return_value = ["model"]
    with (
        patch("blackcortex_cli.commands.OpenAI", return_value=mock_client),
        patch("blackcortex_cli.commands.console.print") as mock_print,
    ):
...
def test_command_ping_failure():
    ...
    with (
        patch("blackcortex_cli.commands.OpenAI", side_effect=commands.OpenAIError("invalid key")),
        patch("blackcortex_cli.commands.console.print") as mock_print,
    ):
